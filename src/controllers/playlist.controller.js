...
const createPlaylist = asyncHandler(async (req, res) => {
    ...
    if(!(name || description) || !(name.trim() !== "" || description.trim() !== "")) throw new ApiError(401, "name and description required!");
...
const getPlaylistById = asyncHandler(async (req, res) => {
        ...
        [
            ...
            },
            {
                $lookup: {
                    ...
                    pipeline: [
                        {
                            $match: { deleted: { $ne: true }} // filtered out deleted videos
                        },
                        {
                            $lookup: {
                                from: "users",
    ...
    return res.status(200).json(
        new ApiResponse(201, playlistAggregate, "playlist fetched successfully!")
    )
})
...
const addVideoToPlaylist = asyncHandler(async (req, res) => {
    ...
    if (!updatePlaylist) throw new ApiError(500, "Playlist not updated");
    return res.status(200).json(
        new ApiResponse(200, updatePlaylist, "playlist updated succesfully!")
    )
})
...
const removeVideoFromPlaylist = asyncHandler(async (req, res) => {
    ...
    const isVideoPlaylist = await Playlist.findOne({
        _id: playlistId,
        videos: videoId
    })
    if (!isVideoPlaylist) throw new ApiError(500, "video not found in playlist");
    ...
    if (!removevideoPlaylist) throw new ApiError(500, "Playlist not updated");
...
const deletePlaylist = asyncHandler(async (req, res) => {
   ...
   if (!deletePlaylist) throw new ApiError(500, "playlist not deleted");
...
const updatePlaylist = asyncHandler(async (req, res) => {
    ...
    if (!updatePlaylist) throw new ApiError(404, "Playlist not updated");
    return res.status(200).json(
